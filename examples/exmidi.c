...
int main() {
  ...
  the_music = load_midi("assets/test.mid");
  if (!the_music) {
  ...
  while (!done) {
    ...
    while (keypressed()) {
      ...
      if (k == 'p') {
        ...
        if (paused) {
          midi_pause();
          textprintf_centre_ex(screen, font, x, y + th * 3, text_color, -1,
                               "P A U S E D");
        } else {
          midi_resume();
          rectfill(screen, x - tw / 2, y + th * 3, x + tw / 2, y + th * 4,
                   background_color);
        }
      } else
        done = 1;
    ...
    textprintf_centre_ex(screen, font, x, y, text_color, -1,
                         "%d:%02d / %d:%02d", pos / 60, pos % 60, length / 60,
                         length % 60);
